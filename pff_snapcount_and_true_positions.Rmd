...
7. Get roster data from nflfastR
8. Join PFF and nflfastR data to get true positions
9. Celebrate
...
# saveRDS(tibble::tibble(), "pff_games2019.rds")
...
# temp <- readRDS("snapcount2020_new.rds")
# max_wk <- max(temp$wk)
# snaps_week <- max_wk+1
...
saveRDS(temp,paste0("pff_main_data",years,".rds"))
...
temp3 <- temp2 %>% 
  filter(game_id==13792)# %>% 
  group_by(game_id,player_id) %>% 
  summarise(n=n()) %>% 
  right_join(temp, by = c("player_id"="id",
                          "game_id" = "game_id")) %>% 
  filter(game_id==13792)

temp4 <- temp %>% 
  filter(game_id==13867)

temp5 <- pbp_db %>% 
  filter(season==2019, week==7, posteam=="LV") %>% 
  group_by(receiver) %>% 
  summarise(n=n()) %>% 
  collect()
...
# main_pff_df <- readRDS("pff_player-games.rds") %>%
#   select(-c("game", "away_team_name", "home_team_name", "status")) %>%
#   left_join(
#   readRDS("snapcount2020_new.rds"),
#   by = c(
#   'player_id' = 'player',
#   "season" = "season",
#   "week" = "wk"
...
#     nflfastR_tm_code = case_when(
#       nflfastR_tm_code == 'LAR' & season <= 2015 ~ 'STL',
